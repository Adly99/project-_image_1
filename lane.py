import cv2                               # enable computer vision
...
filename = 'project_video.mp4'  # Make sure the video file is in the same directory as your code
file_size = (1280,720) # Assumes 1280x720 .. mp4
...
class Lane:
  ...
  def __init__(self, orig_frame):
    ...
    self.orig_image_size = self.orig_frame.shape[::-1][1:]    # (Width, Height) of the original video frame (or image)
  ...
  def calculate_car_position(self, print_to_terminal=False):
    ...
    height = self.orig_frame.shape[0]    # Fine the x coordinate of the lane line bottom
  ...
  def display_curvature_offset(self, frame=None, plot=False):
    ...
    cv2.putText(image_copy,'Center Offset: '+str(self.center_offset)[:7]+' cm', (int((5/600)*self.width), int((40/338)*self.height)), cv2.FONT_HERSHEY_SIMPLEX, (float((0.5/600)*self.width)),(255,255,255),2,cv2.LINE_AA)
  ...
  def get_lane_line_previous_window(self, left_fit, right_fit, plot=False):
    ...
    if plot==True:
      ...
      # Plot the figures "not important"
  ...
  def get_lane_line_indices_sliding_windows(self, plot=False):
    ...
    if plot==True:
      ...
      out_img[nonzeroy[left_lane_inds], nonzerox[left_lane_inds]] = [255, 30, 0]
      out_img[nonzeroy[right_lane_inds], nonzerox[right_lane_inds]] = [0, 50, 255]

      # Plot the figure with the sliding windows "not important"
      figure, (ax1, ax2, ax3) = plt.subplots(3,1) # 3 rows, 1 column
      figure.set_size_inches(10, 10)
      figure.tight_layout(pad=3.0)
      ax1.imshow(cv2.cvtColor(self.orig_frame, cv2.COLOR_BGR2RGB))
  ...
  def get_line_markings(self, frame=None):
    ...
    # Convert the video frame from BGR (blue, green, red) to color space to HLS (hue, saturation, lightness).
...
def main():
  ...
  while cap.isOpened():
    ...
    if success:
      ...
      # Display frame for 1 milliseconds and check if s key is pressed / s == stop
